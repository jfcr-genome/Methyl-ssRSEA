...
### input ###
beta_value_path = commandArgs(trailingOnly=TRUE)[1]
region_set_path = commandArgs(trailingOnly=TRUE)[2]
probe_coord_path = commandArgs(trailingOnly=TRUE)[3]
output_path = commandArgs(trailingOnly=TRUE)[4]

beta_value <- read_csv(beta_value_path)
...
### calculate scores ###
X <- beta_value %>% arrange(Probe) %>% column_to_rownames("Probe") %>% as.matrix()
...
### output ###
write_csv(as.data.frame(t(ssRSEA_scores)), output_path)
